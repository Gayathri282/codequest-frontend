...
export default function Register() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      toast.success('Welcome to GuppyStore!');
      navigate('/');
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12"
      ...
      <div className="relative w-full max-w-sm rounded-3xl px-8 py-10" style={{
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-white/35 text-sm mt-1">Join the GuppyStore family</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input type="password" value={form.password} onChange={set('password')}
              placeholder="Min 6 characters" minLength={6} required className="input" />
          </div>
        ...
        <p className="text-center text-white/30 text-sm mt-6">
          ...
          <Link to="/login" className="font-semibold" style={{ color: '#D4AF37' }}>Sign in</Link>
        </p>
      </div>
    </div>
  );
